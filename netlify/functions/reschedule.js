...
// ---- Store-credit issuing (mirrors credit-issue.js record + email exactly) ----
const SITE = "https://littlehavenplay.com";

async function makeCredit(type, amountCents, reason) {
  ...
  const now = new Date(); const exp = new Date(now); exp.setMonth(exp.getMonth() + months);
  const rec = {
    ...
    createdAt: now.toISOString(), expiry: exp.toISOString().slice(0, 10), active: true,
...
async function sendCreditEmail(to, rec, isOwner) {
  const key = process.env.RESEND_API_KEY;
  const from = process.env.EMAIL_FROM || "onboarding@resend.dev";
  const studio = STUDIO_NAME || "Little Haven Play Studio";
  if (!key) return false;
  ...
  if (isOwner) {
    ...
    html = `
    <div style="font-family:Arial,Helvetica,sans-serif;color:#2a2622;max-width:640px;line-height:1.6">
      <img src="${SITE}/assets/courtesy-credit.png" alt="${esc(studio)} Courtesy Credit" style="width:100%;max-width:640px;border-radius:14px;display:block"/>
      ${codeBlock}
      <p style="font-size:14px">Please enjoy <b>${money}</b> toward a future online open-play booking at ${esc(studio)}.</p>
      ...
      <ol style="font-size:14px;margin:0;padding-left:20px;color:#5c6470">
        <li>Visit littlehavenplay.com</li>
        <li>Select your open play session</li>
        <li>Proceed to checkout</li>
        <li>Enter your credit code in the <b>Store Credit</b> field</li>
        <li>The credit applies automatically to your booking</li>
      </ol>
      <p style="font-size:13px;margin:14px 0 4px"><b>Important</b></p>
      <ul style="font-size:13px;margin:0;padding-left:20px;color:#8a8276">
        <li>One-time use only</li>
        <li>No cash value</li>
        <li>Any unused balance is forfeited after redemption</li>
        <li>Valid for online open-play bookings only</li>
        <li>Expires 3 months from the date of issuance</li>
      </ul>
    ...
    <div style="font-family:Arial,Helvetica,sans-serif;color:#2a2622;max-width:520px;line-height:1.6">
      ...
      <p style="font-size:13px;color:#8a8276">Expires 3 months from the date of issuance.</p>
      <p style="margin-top:14px;background:#fcfaf6;border:1px solid #efe7da;border-radius:10px;padding:11px 13px;font-size:13px;color:#5c6470"><b>Don't see this?</b> Please check your junk/spam folder.</p>
    </div>`;
  }
  try {
